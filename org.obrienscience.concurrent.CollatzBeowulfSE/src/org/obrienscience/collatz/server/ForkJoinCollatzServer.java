...
public class ForkJoinCollatzServer {
	...
	public void compute(long pool, long threads, long uowSplit, long extent) {
		...
		ForkJoinUnitOfWork fj = new ForkJoinUnitOfWork(uowSplit, 0, extent, maxPath, maxValue);
		...
		mapReducePool.invoke(fj);
		long endTime = System.currentTimeMillis();
		System.out.println(new StringBuffer(
				String.valueOf(endTime - startTime)).append(",")
				.append(pool).append(",")
				.append(threads).append(",")
				.append(String.valueOf(endTime - startTime)).append(" ms for Collatz ")
				.append("uow^2: ")
				.append(threads).append(" split: ").append(uowSplit).toString());
	}
	
	public static void main(String[] args) {
	    System.out.println("ForkJoinCollatzServer forkJoinPool-power");
		long poolStart = 0;
		long poolEnd = 7;
